...
L'utilisation du webhook nous montre l'architecture client-serveur évenementielle, où TTN agit comme un serveur client envoyant des requêtes POST vers notre serveur.
...
![Terminal JSON vs Protocol Buffers](img/9compare.png)
...
##### Est-ce normal?
Non. Un vrai bridge en couche 2 devrait relayer les trames mais conserver l'adresse MAC source originale de l'expéditeur.

##### Que se passe-t-il probablement?

Les access points en mode "bridge" fonctionnent plutôt en mode **relais avec réécriture MAC** (MAC rewriting/source MAC replacement):
...
Format d'une trame 802.11:
![Format d'une trame 802.11 venant du support de cours "technologies-wifi](img/format_trame_802_11.jpg)
-
...
On peut appliquer des milliers/millions d’itérations, parfois avec coût mémoire élevé, pour dériver une clé AES à partir du mot de passe. Cela ralentit fortement les attaques par brute force car il augmente le coût de calcul pour l'attaquant.
...
![Schéma en couche 2](img/schema_couche_2.jpg)
![Schéma en couche 3](img/schema_couche_3.jpg)
-
...
En agissant ainsi, même si le wifi était intercepté, les données resteraient difficiles à lire sans la clé de chiffrement.

Ce n'est pas idéal, car le réseau Wifi reste ouvert dans la couche 2 (liaison). En effet, sans chiffrement en couche 2, toutes les trames wifi sont visibles.
De plus, les capteurs IOT ont souvent peu de ressources. Le chiffrement applicatif en demande beaucoup.
...
## Annexes
[payload-decode.js](code/payload-decode.js)

[script.php](code/script.php)

[VLAN ids et configurations trunk](https://jumpcloud.com/it-index/what-is-vlan-id)
